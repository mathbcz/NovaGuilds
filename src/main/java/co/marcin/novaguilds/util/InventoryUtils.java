...
public final class InventoryUtils {
    ...
    public static void removeItems(Player player, List<ItemStack> items) {
        items.stream().filter(item -> player.getGameMode() != GameMode.CREATIVE || item.hasItemMeta())
                .forEachOrdered(item -> player.getInventory().removeItem(item));
    }
    ...
    public static int getTotalAmountOfItemStackInInventory(Inventory inventory, ItemStack itemStack) {
        return Arrays.stream(inventory.getContents())
                .filter(item -> item != null && item.getType() != Material.AIR && ItemStackUtils.isSimilar(itemStack, item))
    ...
    public static boolean isEmpty(Inventory inventory) {
        return Arrays.stream(inventory.getContents()).noneMatch(itemStack -> itemStack != null && itemStack.getType() != Material.AIR);
    }
