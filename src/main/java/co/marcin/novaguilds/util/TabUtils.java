...
public final class TabUtils {
    ...
    public static void refresh(final NovaPlayer nPlayer) {
        ...
        if (Bukkit.isPrimaryThread()) {
            new Thread(() -> nPlayer.getTabList().send()).start();
        } else {
    ...
    public static void refresh(final List<NovaPlayer> list) {
        new Thread(() -> list.forEach(TabUtils::refresh)).start();
    }
    ...
    public static void fillVars(TabList tabList) {
        ...
        int onlinePlayersCount = (int) CompatibilityUtils.getOnlinePlayers().stream()
                .filter(player -> !plugin.getPlayerManager().isVanished(player))
                .count();
